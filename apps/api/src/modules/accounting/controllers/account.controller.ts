...
export class AccountController {
    ...
    async create(
        @Request() req: any,
        @Body() dto: { code: string; name: string; type: AccountType; details?: string; parentId?: string }
    ...
    async seed(@Request() req: any): Promise<ApiResponseDto<{ message: string }>> {
        await this.service.seed(req.user.tenantId);
        return createSuccessResponse({ message: 'Core accounts seeded successfully' }, 201);
    }
}
