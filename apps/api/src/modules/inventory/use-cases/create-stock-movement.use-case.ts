...
export class CreateStockMovementUseCase {
    ...
    async execute(tenantId: string, userId: string, dto: CreateStockMovementDto) {
        ...
        const updates: { tenantId: string, warehouseId: string, variantId: string }[] = [];
        ...
        const movement = await this.prisma.$transaction(async (tx) => {
            ...
            const createdMovement = await this.stockRepo.createMovement(movementData, tx);

            for (const item of movementData.items) {
                if (dto.type === MovementType.OUTBOUND || dto.type === MovementType.TRANSFER) {
                    if (!dto.fromWarehouseId) throw new BadRequestException('From Warehouse required');
                    await this.adjustStock(dto.fromWarehouseId, item.productVariantId, -item.quantity, item.batchId, tx);
                    updates.push({ tenantId, warehouseId: dto.fromWarehouseId, variantId: item.productVariantId });
                }

                if (dto.type === MovementType.INBOUND || dto.type === MovementType.TRANSFER) {
                    if (!dto.toWarehouseId) throw new BadRequestException('To Warehouse required');
                    await this.adjustStock(dto.toWarehouseId, item.productVariantId, item.quantity, item.batchId, tx);
                    updates.push({ tenantId, warehouseId: dto.toWarehouseId, variantId: item.productVariantId });
                }
            }
