...
export class WarehouseController {
    ...
    async findOne(@Request() req: any, @Param('id') id: string): Promise<ApiResponseDto<any>> {
        const warehouse = await this.warehouseService.findOne(id, req.user.tenantId);
    ...
    async update(
        ...
        const warehouse = await this.warehouseService.update(id, req.user.tenantId, updateWarehouseDto);
        return createSuccessResponse(warehouse);
    }
