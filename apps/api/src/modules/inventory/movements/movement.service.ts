...
import { CreateStockMovementDto, MovementType, MovementStatus, StockMovementDto } from '@repo/dto';

@Injectable()
export class MovementService {
    constructor(private prisma: PrismaService) { }

    async create(tenantId: string, userId: string, dto: CreateStockMovementDto): Promise<StockMovementDto> {
        return this.prisma.$transaction(async (tx) => {
            // 1. Validation
            const { type, fromWarehouseId, toWarehouseId, items, reference } = dto;

            if (type === MovementType.INBOUND && !toWarehouseId) {
                throw new BadRequestException('Inbound movement requires toWarehouseId');
            }
            if (type === MovementType.OUTBOUND && !fromWarehouseId) {
                throw new BadRequestException('Outbound movement requires fromWarehouseId');
            }
            if (type === MovementType.TRANSFER && (!fromWarehouseId || !toWarehouseId)) {
                throw new BadRequestException('Transfer movement requires fromWarehouseId and toWarehouseId');
            }
            ...
            for (const item of items) {
                ...
                if (item.batchCode) {
                    ...
                    if (batch) {
                        batchId = batch.id;
                    } else if (type === MovementType.INBOUND) { // Only create batch on INBOUND
                        ...
                        throw new BadRequestException(`Batch ${item.batchCode} not found for product`);
                    }
                }
                ...
                if (type === MovementType.INBOUND) {
                    await this.increaseStock(tx, toWarehouseId!, item.productVariantId, batchId, item.quantity);
                } else if (type === MovementType.OUTBOUND) {
                    await this.decreaseStock(tx, fromWarehouseId!, item.productVariantId, batchId, item.quantity);
    ...
    private async increaseStock(tx: any, warehouseId: string, variantId: string, batchId: string | null, infoQty: number) {
    ...
    private async decreaseStock(tx: any, warehouseId: string, variantId: string, batchId: string | null, infoQty: number) {
        const existing = await tx.stock.findFirst({
            where: {
                warehouseId,
                productVariantId: variantId,
        ...
        if (!existing || existing.quantity < infoQty) {
            throw new BadRequestException(`Insufficient stock for variant ${variantId} at warehouse ${warehouseId}`);
        }
