  ...
  async create(tenantId: string, createdById: string, data: CreateMovementDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const totalValue = data.items.reduce(
        (sum, item) => sum + item.quantity * 100000,
        0,
      );
  ...
  private async adjustStock(
    ...
    if (stock) {
      if (stock.quantity + quantity < 0) {
        throw new BadRequestException(
          `Insufficient stock for variant ${productVariantId} (Batch: ${batchId || 'N/A'}) in warehouse ${warehouseId}`,
        );
      }
      ...
      if (quantity < 0) {
        throw new BadRequestException(
          `Insufficient stock (not found) for variant ${productVariantId} (Batch: ${batchId || 'N/A'}) in warehouse ${warehouseId}`,
        );
      }
