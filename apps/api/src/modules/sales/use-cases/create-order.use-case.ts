    ...
    async execute(tenantId: string, userId: string, dto: CreateOrderDto) {
        ...
        return this.prisma.$transaction(async (tx: any) => {
            ...
            for (const item of orderItems) {
                ...
                if (stocks.length === 0) throw new BadRequestException(`Out of stock: ${item.productVariantId}`);

                // Simple strategy: Take from first.
                const sourceStock = stocks[0];
                if (sourceStock.quantity < remainingToDeduct) {
                    throw new BadRequestException(`Not enough stock in single warehouse for ${item.productVariantId} (Has ${sourceStock.quantity}, Need ${remainingToDeduct})`);
                }
                ...
                stockUpdates.push({ tenantId, warehouseId: sourceStock.warehouseId, variantId: item.productVariantId });
            }

            return { order, stockUpdates };
        }).then(result => {
            // Post transaction events
            result.stockUpdates.forEach(u => this.eventEmitter.emit('stock.updated', u));
