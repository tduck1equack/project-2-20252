...
export class ProductController {
    ...
    async create(
        @Request() req: any,
        @Body() createProductDto: { name: string; sku: string; description?: string; uomId: string; variants?: any[] }
    ): Promise<ApiResponseDto<any>> {
        const product = await this.productService.create({ ...createProductDto, tenantId: req.user.tenantId });
    ...
    async update(
        @Request() req: any,
        @Param('id') id: string,
        @Body() updateProductDto: { name?: string; description?: string; uomId?: string }
    ): Promise<ApiResponseDto<any>> {
        const product = await this.productService.update(id, req.user.tenantId, updateProductDto);
        return createSuccessResponse(product);
    }
